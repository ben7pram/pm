  ...
  useEffect(() => {
    const fetchBoard = async () => {
      try {
        ...
        // Fall back to initialData so UI still works
  ...
  const saveBoard = async (updatedBoard: BoardData) => {
    ...
    try {
      const resp = await fetch("/api/board", {
        ...
        body: JSON.stringify({ data: updatedBoard }),
      });
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-[var(--gray-text)]">Loading board...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="rounded-full border border-[var(--stroke)] px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--navy-dark)] hover:bg-[var(--surface)]"
              >
                Logout
              </button>
            </div>
